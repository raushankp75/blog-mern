                    ...
                    <div className='flex flex-row items-center'>
                        {/* toggle profile icon open and close */}
                        {
                            userData.map((user, index) => {
                                return (
                                    <div className='relative md:right-0 right-16 cursor-pointer' key={index}>
                                        <button onClick={() => setOpenProfile(!openProfile)} className='flex flex-row items-center gap-2 text-gray-200 hover:text-blue-400'>
                                            <img src={user?.image?.url ? user?.image?.url : 'UserImage'} alt="im" className='md:w-12 md:h-12 w-8 h-8 rounded-full bg-white' />
